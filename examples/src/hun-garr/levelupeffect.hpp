...
#ifndef __LEVELUP_EFFECT_HPP__
#define __LEVELUP_EFFECT_HPP__
#ifdef _WIN32
#pragma once
#endif
...
struct BouncyChar
{
	...
	BouncyChar(PopString t, float x, float y, float s)
	{
		...
		mX = x;
		mY = y;
		mBounceSpeed = mOldBounceSpeed = s;
		mDone = false;
	}
};
...
class LevelupEffect
{
  ...
  private:
	...
	bool mStartNextLevel; // When true, indicates that the board should set up the next level so that it'll be there
						  // when we fade out
  ...
  public:
	...
	bool HideBoard()
	{
		return (mState == SHOWING_STATS) || (mState == CURTAIN_OUT) || (mState == COVERING_STATS);
	}
	...
	bool HidePlanets()
	{
		return IsActive() && (HideBoard() || (mState == LEVELUP_TEXT) || (mState == CURTAIN_IN));
	}
};
