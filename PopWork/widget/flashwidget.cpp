...
struct __declspec(uuid("a6ef9860-c720-11d0-9337-00a0c90dcaa9")) IDispatchEx : IDispatch
{
	...
	HRESULT RemoteInvokeEx(long id, unsigned long lcid, unsigned long dwFlags, struct DISPPARAMS *pdp, VARIANT *pvarRes,
						   struct EXCEPINFO *pei, struct IServiceProvider *pspCaller, unsigned int cvarRefArg,
...
class ControlSite : public IOleInPlaceSiteWindowless, public IOleClientSite
{
  ...
  public:
	...
	HRESULT STDMETHODCALLTYPE GetDC(/* [in] */ LPCRECT pRect, /* [in] */ DWORD grfFlags,
	...
	HRESULT STDMETHODCALLTYPE ScrollRect(/* [in] */ INT dx, /* [in] */ INT dy, /* [in] */ LPCRECT pRectScroll,
	...
	HRESULT STDMETHODCALLTYPE OnDefWindowMessage(/* [in] */ UINT msg, /* [in] */ WPARAM wParam,
												 /* [in] */ LPARAM lParam, /* [out] */ LRESULT __RPC_FAR *plResult)
	{
		return S_FALSE;
	}
};
...
FlashWidget::FlashWidget(int theId, FlashListener *theFlashListener)
{
	...
	mPauseCount = 0;
	mAutoPause = true;
	mHasLostFocus = !gAppBase->mHasFocus;
	mFlashDirty = true;
...
void FlashWidget::RebuildImages()
{
	...
	mImage = new SDLImage(gAppBase->mSDLInterface);
...
void FlashWidget::CheckCursor()
{
	...
	{
		mCurCursor = CURSOR_POINTER;
		// gAppBase->mOverrideCursor = NULL;
		// gAppBase->SetCursor(mCurCursor);
	}
	else if (aCursor != NULL)
	{
		// gAppBase->mOverrideCursor = aCursor;
...
void FlashWidget::MouseDown(int x, int y, int theBtnNum, int theClickCount)
{
	if (theBtnNum == 0)
	{
		LRESULT aResult;
		Point anAbsPos = GetAbsPos();
		mWindowlessObject->OnWindowMessage(WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(x + anAbsPos.mX, y + anAbsPos.mY),
										   &aResult);
		CheckCursor();
	}
}
...
void FlashWidget::MouseDrag(int x, int y)
{
	if (mWidgetManager->IsLeftButtonDown())
	{
		LRESULT aResult;
		Point anAbsPos = GetAbsPos();
		mWindowlessObject->OnWindowMessage(WM_MOUSEMOVE, MK_LBUTTON, MAKELPARAM(x + anAbsPos.mX, y + anAbsPos.mY),
										   &aResult);
		CheckCursor();
	}
}
...
void FlashWidget::MouseLeave()
{
	...
	gAppBase->mOverrideCursor = NULL;
	gAppBase->SetCursor(CURSOR_POINTER);
}
