...
void TextWidget::DrawColorStringHilited(Graphics* g, const PopWorkString& theString, int x, int y, int theStartPos, int theEndPos)
{
	...
	else
	{
		...
		while (aCurPos < (int)theLine.length())
		{
			...
			if (GetColorStringWidth(aNewString) > mWidth-8)
			{
				...
				aCurString = _S("  ") + PopWorkChar(0xFF) + (PopWorkChar) aColor.mRed + (PopWorkChar) aColor.mGreen + (PopWorkChar) aColor.mBlue +
