...
public:
	...
	PopWorkAutoPerf(const char *theName) : mName(theName), mIsStarted(true) { PopWorkPerf::StartTiming(theName); }
...
#define POPWORK_AUTO_PERFL(theName) POPWORK_AUTO_PERF_2(theName,__LINE__) // __LINE__ doesn't work correctly if Edit-and-Continue (/ZI) is enabled
#define POPWORK_AUTO_PERF(theName) POPWORK_AUTO_PERF_2(theName,UNIQUE)

#define POPWORK_PERF_BEGIN_COND(theName,theCond) if(theCond) PopWorkPerf::StartTiming(theName)
#define POPWORK_PERF_END_COND(theName,theCond) if(theCond) PopWorkPerf::StopTiming(theName)
#define POPWORK_AUTO_PERF_MULTI_COND(theName,theSuffix,theCond) PopWorkAutoPerf anAutoPerf##theSuffix(theName,theCond); 
