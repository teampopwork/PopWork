...
void SDLTextureData::CreateTextures(MemoryImage *theImage)
{
	...
	if (createTexture)
	{
		...
		SDL_SetTextureScaleMode(mTexture, (SDL_GetRendererName(mRenderer) == "software" ||
										   theImage->mImageFlags & SDLImageFlag_NearestFiltering)
							  ...
							  const Color &theColor, int theDrawMode, bool fastStretch, bool mirror)
{
	...
	SDL_FRect destRect = {(float)theDestRect.mX, (float)theDestRect.mY, (float)theDestRect.mWidth * xScale,
	...
	SDL_SetRenderClipRect(mRenderer, &clipRect);
	SDL_SetTextureBlendMode(aTexture, ChooseBlendMode(theDrawMode));
	SDL_RenderTextureRotated(mRenderer, aTexture, &srcRect, &destRect, 0, NULL,
							...
							int theDrawMode)
{
	...
	SDL_SetRenderDrawBlendMode(mRenderer, ChooseBlendMode(Graphics::DRAWMODE_NORMAL)); // RESET cause this aint a
																					   // texture
	SDL_SetRenderTarget(mRenderer, nullptr);
}
